...
export default function EditEventTypePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (eventType) {
      reset({
        ...
        dailyLimit: eventType.dailyLimit || undefined,
        color: eventType.color,
        defaultVideoLink: eventType.defaultVideoLink || '',
      });
  ...
  const updateMutation = useMutation({
    mutationFn: (data: EventTypeForm) => {
      ...
      return eventTypeApi.update(id, payload);
    },
    onSuccess: () => {
  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">Edit Event Type</CardTitle>
            <CardDescription>
